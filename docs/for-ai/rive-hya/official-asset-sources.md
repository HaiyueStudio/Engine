...
后续回归把响应式入场和循环阶段进一步分离：放大/位移采用父 `ForGridPlanet` 布尔 transition 的作者值 `200 ms`，并按 nested artboard 高度抵消缩放产生的垂直漂移，只保留向右扩张；组合时间轴保存 `loopStart=1 s`，播放器首轮执行 `NeptuneIn`，之后只在完整的 6 秒 `NeptuneIdle` 区间循环。因此第二轮表情变化不会重放入场位移、放大或旋转，pointer exit 仍归零并恢复 idle tree。`loopStart` 是通用 HYA 播放区间能力，但 transition 查找与响应式位移仍只闭合该确定性 boolean/nested-fit 子图。
