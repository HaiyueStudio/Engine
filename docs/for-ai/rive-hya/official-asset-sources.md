...
本轮未提交工作树上的三项定向诊断进一步缩小了 G10 blocker。`official-text-fit` 的 34 个重放采样全部通过 pixel validator，最大通道差为 `1/255`、changed pixel ratio 为 `0`、最低 SSIM 为 `0.9999994597`；这关闭了该素材的本地 implementation blocker，但仍需在后续 clean revision 上重跑正式 Chrome/Edge trace。`official-text-style-background` 的 style run、joined background bounds 与字体 outline 已基本重合，仍因每帧 `3–6` 个字形边缘覆盖采样不同而失败，最高 changed pixel ratio 为 `0.00146484375`，不能降低阈值后宣称通过。`official-inventory-demo-v2` 在 nested layout participant 与 bounded gradient-feather lowering 后，最高 changed pixel ratio 从 `0.306640625` 降至 `0.28369140625`、最低 SSIM 从 `0.965686095` 提升至 `0.9659616981`；但 per-paint signed-distance feather 尚未进入 HYA vector runtime，因此 backpack shell paint blocker 仍保留。以上均为 dirty diagnostic，不计入 G11 formal evidence。
