...
同一官方素材的后续差异回归继续闭合四项可复用语义。第一，nested-fit hover 使用解析后的 scale 与垂直中心补偿；水平侧只补运行时缩放锚点尚未覆盖的剩余位移，避免完整补偿叠加两次。第二，`LayoutComponent.clip=true` 与 style 的 linked corner radius 现在生成可执行 alpha mask，并传播到容器子树，超出圆角卡片的绘制会被裁掉。第三，`InterpolatingKeyFrame.interpolationType` 的省略值按 Rive schema 默认 Hold（0）解释，`LoopIce` 的轮廓 rotation 因而生成 STEP track，不再虚构线性中间姿态；普通点路径形变保持连续插值。第四，`Drawable.blendModeValue=14` 映射为 HYA `screen`，播放器以 WebGPU screen 混合还原 Vapor 呼气，并由布局 mask 和原 opacity timeline 共同约束出现、消失与越界区域。Feature Corpus 仍维持这些 object/property 为 `partial`，因为这里只闭合官方素材命中的 screen/clip/STEP 子集，尚未覆盖全部 blend、layout reflow 与状态机组合。

后续回归把响应式入场、循环与退出阶段进一步分离：放大/位移采用父 `ForGridPlanet` 布尔 transition 的作者值 `200 ms`，并按 nested artboard 高度抵消缩放产生的垂直漂移，只保留向右扩张与原路回收；组合时间轴保存 `loopStart=1 s`、`loopEnd=7 s`、`exitStart=7 s`，播放器只循环完整的 6 秒 `NeptuneIdle`，离开 hover 后再单次播放约 1 秒的退出尾段。因此第二轮表情变化不会重放入场位移、放大或旋转，离开也不再瞬间复位。素材中 `LoopIce` 省略插值字段的 rotation key 作为 Hold/STEP 保留官方轮廓的间隔跳跃感，表情与 Vapor 的普通 `PointsPath` 形变则维持连续插值。播放区间是通用 HYA 能力，但 transition 查找与响应式位移仍只闭合该确定性 boolean/nested-fit 子图。

本轮未提交工作树上的三项定向诊断进一步缩小了 G10 blocker。`official-text-fit` 的 34 个重放采样全部通过 pixel validator，最大通道差为 `1/255`、changed pixel ratio 为 `0`、最低 SSIM 为 `0.9999994597`；这关闭了该素材的本地 implementation blocker，但仍需在后续 clean revision 上重跑正式 Chrome/Edge trace。`official-text-style-background` 的 style run、joined background bounds 与字体 outline 已基本重合，仍因每帧 `3–6` 个字形边缘覆盖采样不同而失败，最高 changed pixel ratio 为 `0.00146484375`，不能降低阈值后宣称通过。`official-inventory-demo-v2` 的 gradient inner Feather 已进入 HYA binary/runtime/WebGPU 执行链；与旧 gradient-stroke 代理相比，最低 SSIM 从 `0.9597800093` 提升到 `0.9663646920`，首帧 HYA/official RGB 总量也明显收敛，但最高 changed pixel ratio 从 `0.28369140625` 升到 `0.302734375`，说明边缘 coverage 仍未达到 Rive 的精确 Gaussian 结果。实色 feather 暂留有界 stroke 代理，paint blocker 仍保留。以上均为 dirty diagnostic，不计入 G11 formal evidence。
