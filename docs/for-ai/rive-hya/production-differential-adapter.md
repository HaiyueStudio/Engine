...
可用以下命令生成单一 host 配置，避免手工维护六组 command/descriptor；三个 provider 路径必须指向实际设备实现：

```powershell
npm run rive:g11:host-config -- --capability-provider=<capability-provider.mjs> --official-provider=<official-provider.mjs> --hya-provider=<hya-provider.mjs> --out=artifacts/rive-g11-formal/host-config.json
...
2. 在每台物理机器的 Chrome、Edge 环境各运行一次 device matrix；每次自动完成 8 个素材，所以两台机器共四次、最终仍是 32 条 trace：
   ...
   runner 会先执行三 host preflight，再从 manifest 的官方不可变 URL 下载 8 个素材并逐项验证 byte length/SHA-256；若已下载，可用 `--source-dir=<directory>` 按官方原始文件名读取本地缓存。`.riv` 只进入系统临时目录，成功或失败后都删除。每条 trace 保留全部 channel/RGBA artifact，并同时写出 `animation.hya`、`animation.hyapkg`、`conversion-manifest.json` 与 `conversion-report.json`，可直接交给 `examples/rive-hya-compare` 做同源左右检查；`artifacts/` 已由 Git 忽略，因此连续运行不会污染 clean source identity。底层单条调试仍可直接使用 `rive-run-differential-trace.mjs --formal`。
3. 在同一 clean revision 运行 `node scripts/benchmark/rive-g11-run-security.mjs --formal --out=artifacts/rive-g11-formal/security.json`。随后用 `rive-browser-closure-scan.mjs --formal` 同时传入 `--packed-player`、`--browser-bundle`、`--source-map`、`--network-log`，并把 `--out` 指向 `artifacts/rive-g11-formal/browser-closure.json`。闭包报告会重新约束 Node.js 22+、clean revision、四项全通过和零禁止项；dirty diagnostic 报告不能被 collector 接受。
