...
const result = await runChromeWebGpuFixture({ root, fixture: 'scripts/webgpu-gate/auxiliary-semantics-fixture.html', timeoutMs: 60_000 });
if (createPerformanceSourceFingerprint(root, root) !== sourceFingerprint) throw Error('Auxiliary semantics inputs changed during verification; retry with stable inputs.');
const output = resolve(root, 'artifacts/webgpu/auxiliary-semantics-diagnostic.json');
...
console.log(`[auxiliary-semantics] passed: ${result.cases.length} pixel cases, zero validation errors. ${output}`);
