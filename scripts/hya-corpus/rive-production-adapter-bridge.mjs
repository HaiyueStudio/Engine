...
const PROTOCOL = 'haiyue-rive-production-adapter@1';
...
export async function invokeProductionHost({ operation, descriptor, request, signal, command, args = [], timeoutMs = 120_000, maxOutputBytes = DEFAULT_MAX_OUTPUT_BYTES }) {
  ...
  const input = Buffer.from(`${JSON.stringify(encodeWire({ protocol: PROTOCOL, operation, descriptor, request }))}\n`);
  const stdout = await run(command, args, input, { signal, timeoutMs, maxOutputBytes });
  let response;
  try { response = decodeWire(JSON.parse(stdout.toString('utf8'))); }
...
function validateHostResponse(response, operation, descriptor) {
  if (response?.protocol !== PROTOCOL || response?.operation !== operation || response?.status !== 'completed') {
...
function encodeWire(value, seen = new Set()) {
  ...
  if (value instanceof Map) return { $haiyueMap: [...value].map(([key, item]) => [key, encodeWire(item, seen)]) };
  ...
  seen.add(value);
  const output = Object.fromEntries(Object.entries(value).filter(([key]) => key !== 'signal').map(([key, item]) => [key, encodeWire(item, seen)]));
  seen.delete(value);
  return output;
}
...
function decodeWire(value) {
  ...
  if (Object.keys(value).length === 1 && Array.isArray(value.$haiyueMap)) return new Map(value.$haiyueMap.map(([key, item]) => [key, decodeWire(item)]));
  return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, decodeWire(item)]));
}
