...
if (result.passCount !== 6 || result.executedPassCount !== 3 || result.computeIrHashCount !== 6) failures.push(`pass evidence=${result.passCount}/${result.executedPassCount}/${result.computeIrHashCount}`);
...
if (result.cache?.shaderModules !== 6 || result.cache?.bindGroupLayouts !== 6 || result.cache?.pipelineLayouts !== 6) failures.push(`cache=${JSON.stringify(result.cache)}`);
