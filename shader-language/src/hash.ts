...
export function sha256Hex(value: string): string {
  ...
  for (let block = 0; block < paddedLength; block += 64) {
    ...
    for (let index = 16; index < 64; index++) {
      const previous15 = word(words, index - 15);
      const previous2 = word(words, index - 2);
      const sigma0 = rotateRight(previous15, 7) ^ rotateRight(previous15, 18) ^ (previous15 >>> 3);
      const sigma1 = rotateRight(previous2, 17) ^ rotateRight(previous2, 19) ^ (previous2 >>> 10);
      words[index] = (word(words, index - 16) + sigma0 + word(words, index - 7) + sigma1) >>> 0;
    }
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const temp1 = (h + sum1 + choose + word(SHA256_CONSTANTS, index) + word(words, index)) >>> 0;
