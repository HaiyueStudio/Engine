...
export class FxaaPass extends PostProcessPass {
  ...
  prepare(device: GPUDevice, format: GPUTextureFormat): void {
    const shader = getBuiltinPostprocessShader(device, 'fxaa');
  ...
  override contributePipelineWarmup(plan: PipelineWarmupPlan, device: GPUDevice): void {
    this.addPipelineWarmup(plan, 'main', 'FXAA', device, () => this._pipelineDescriptor(),
