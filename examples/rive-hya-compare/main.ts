...
async function main(): Promise<void> {
  ...
  const loadOfficial = async (sample: Sample): Promise<void> => {
    ...
    setStatus(hyaPlayer ? '两端均已加载，可视觉对照' : '官方端已加载；等待同源 HYA 转换产物', hyaPlayer ? 'success' : 'working');
    updateResult();
  };

  const installHya = async (hyaBytes: ArrayBuffer, report: ConversionReport, label: string): Promise<void> => {
    ...
    const animation = parseAnimation(hyaBytes);
    if (hyaEntity) scene.remove(hyaEntity);
    hyaPlayer = new Animation2DComponent(animation, { autoplay: playing, loop: true });
  ...
  window.addEventListener('beforeunload', () => { resize.disconnect(); official?.cleanup(); engine.destroy(); }, { once: true });
  await loadOfficial(activeSample);

  function updateResult(): void {
    const result = query<HTMLElement>('#result');
    result.dataset.status = hyaPlayer && official ? 'passed' : 'official-only';
